...
//        }
        ...
        $(document).on('change', '.ays_toggle_checkbox', function (e) {
            ...
            var parent = $(this).parents('.ays_toggle_parent').eq(0);
            
            if($(this).hasClass('ays_toggle_slide')){
                switch (state) {
                    case true:
                        parent.find('.ays_toggle_target').eq(0).slideDown(250);
                        parent.find('.ays_toggle_target_2').eq(0).slideDown(250);

                        parent.find('.ays_toggle_target_inverse').eq(0).slideDown(250);
                        break;
                    case false:
                        parent.find('.ays_toggle_target').eq(0).slideUp(250);
                        parent.find('.ays_toggle_target_2').eq(0).slideUp(250);
                ...
                switch (state) {
                    case true:
                        ...
                        parent.find('.ays_toggle_target_2').eq(0).show(250);

                        parent.find('.ays_toggle_target_inverse').eq(0).hide(250);
                        break;
                    case false:
                        parent.find('.ays_toggle_target').eq(0).hide(250);
                        parent.find('.ays_toggle_target_2').eq(0).hide(250);
...
//        });
        ...
        $(document).on("dblclick" , "#form_available_fields > li", function () {
            var item = $(this);
            var parent = item.parents('#form_available_fields');
            var $default_attributes = ["ays_form_name","ays_form_email","ays_form_phone"];
            item.find('.custom_field_required').removeClass('display_none');
            item.find('input[name="ays_quiz_attributes_passive[]"]').attr('name', 'ays_quiz_attributes[]');
            item.find('input[name="ays_quiz_attributes_passive_order[]"]').attr('name', 'ays_quiz_attributes_active_order[]');
            if( parent.find('li.ui-sortable-helper').length == 0 ){
                for(var i=0; i < $default_attributes.length; i++){
                    item.find('input[name="'+$default_attributes[i]+'"]').val('on');
                }
        ...
        $(document).on("dblclick", '#form_fields > li', function () {
            var item = $(this);
            var parent = item.parents('#form_fields');
            var $default_attributes = ["ays_form_name","ays_form_email","ays_form_phone"];
            item.find('.custom_field_required').addClass('display_none');
            item.find('input[name="ays_quiz_attributes[]"]').attr('name', 'ays_quiz_attributes_passive[]');
            item.find('input[name="ays_quiz_attributes_active_order[]"]').attr('name', 'ays_quiz_attributes_passive_order[]');
...
//        });
        ...
        $('.ays-add-interval').on('click', function () {
            ...
            var wooOptions = "";
            var intervalsOpenEditorImg = quizLangObj.adminUrl + '/images/icons/edit-content.svg';
            ...
            var mailAttachmentHTML = "";

            mailAttachmentHTML += '<div class="ays-quiz-interval-email-message-files-main-contnet">';

                mailAttachmentHTML += '<div class="ays-quiz-interval-condition-containers-conditions-contents-messages-title">';
                        mailAttachmentHTML += '<p class="ays-subtitle ays-quiz-interval-messages-contnet-titles">'+ quizLangObj.intervalEmailAttachment +' ';
                            mailAttachmentHTML += '<a class="ays_help" data-toggle="tooltip" title="'+ quizLangObj.intervalEmailAttachmentTooltip +'">';
                                mailAttachmentHTML += '<i class="ays_fa ays_fa_info_circle"></i>';
                            mailAttachmentHTML += '</a>';
                        mailAttachmentHTML += '</p>';
                mailAttachmentHTML += '</div>';
                mailAttachmentHTML += '<hr>';

                mailAttachmentHTML += '<div class="ays-quiz-interval-email-message-files">';

                    mailAttachmentHTML += '<div>';
                        mailAttachmentHTML += '<button class="button ays-quiz-interval-add-email-message-files" type="button">'+ quizLangObj.intervalAddFile +'</button>';
                        mailAttachmentHTML += '<input type="hidden" class="ays-quiz-interval-email-message-editor-file" name="interval_messages_email_file[]" value="" >';
                        mailAttachmentHTML += '<input type="hidden" class="ays-quiz-interval-email-message-editor-file-id" name="interval_messages_email_file_id[]" value="" >';
                    mailAttachmentHTML += '</div>';


                    mailAttachmentHTML += '<div class="ays-quiz-interval-email-message-files-body display_none">';
                        mailAttachmentHTML += '<div class="ays-quiz-interval-email-message-files-content">';
                            mailAttachmentHTML += '<a href="<?php echo $interval_messages_email_file;?>" download="" class="ays-quiz-interval-email-message-files-content-text"></a>';
                            mailAttachmentHTML += '<input type="hidden" class="ays-quiz-interval-email-message-editor-file-name" name="interval_messages_email_file_name[]" value="" >';
                        mailAttachmentHTML += '</div>';
                    mailAttachmentHTML += '</div>';


                    mailAttachmentHTML += '<div class="ays-quiz-interval-email-message-files-wrapper display_none">';
                        mailAttachmentHTML += '<div class="ays-quiz-interval-email-message-files-wrapper-delete-wrap">';
                            mailAttachmentHTML += '<div role="button" class="ays-quiz-interval-email-message-files-wrapper-delete-cont ays-quiz-interval-email-message-files-remove removeFile">';
                                mailAttachmentHTML += '<div class="ays-quiz-interval-icons">';
                                    mailAttachmentHTML += '<img src="'+ quizLangObj.adminUrl +'/images/remove-normal.png">';
                                mailAttachmentHTML += '</div>';
                            mailAttachmentHTML += '</div>';
                        mailAttachmentHTML += '</div>';
                    mailAttachmentHTML += '</div>';
            ...
            intervals_table.append("<tbody><tr class=\"ays-interval-row ui-state-default " + className + " \">\n" +
                ...
                intervalsHTML +
                "   <td><div class='intervall-text-section-container'>"
                +
                    "<textarea name=\"interval_text[]\" class=\"interval_text\"></textarea>\n" +
                    "<div class='ays-quiz-intervals-img-icon-box'>" +
                        "<div class='ays-quiz-open-intervals-editor ' data-container='body' data-toggle='popover' data-trigger='hover' data-placement='auto' data-content='Open editor' data-original-title='' title=''>" +
                            "<div class='ays-intervals-img-icon-content'> " +
                                "<div class='ays-intervals-img-icon-content-div'>" +
                                    "<div class='ays-quiz-icons'>" +
                                        "<img src='" + intervalsOpenEditorImg + "'>" +
                                    "</div>" +
                                "</div> " +
                            "</div>" +
                        "</div>" +
...
//            trigger: "focus"
        ...
        if ($('#ays-attribute-type').val() == 'file') {
            $('.ays_attr_file_options').show(250);
        }
        $('#ays-attribute-type').on('change', function () {
            if ($(this).val() === 'select') {
                $('.ays_attr_options').show(250);
                $('.ays_attr_description').hide();
                $('.ays_attr_file_options').hide(250);
        ...
        $(document).on("keydown" , "input[name='ays-correct-answer-value[]']" , function(e) {
            ...
            if ( lastAnswer.is(":focus") ) {
                ...
                if (e.keyCode === 13) {
                    ...
                    if (nextElement.length > 0) {
                        nextElement = $( parentTr.next().find("input.ays-correct-answer-value")[0] );
            ...
            if(e.keyCode === 8 && $thisValue == ""){
                ...
                if( questionType == "matching"){
                    var matchInput = $this.parents('tr.ays-answer-row').find('input[name="ays-correct-answer-value-match[]"]');
...
//        }
        ...
        // Quiz Business banner | Start
        var mainBusinessPopupButton = $(document).find(".ays-quiz-popup-box-header-close-button, .ays-quiz-popup-box-header-desc-a, .ays-quiz-popup-box-action-button");
        if( mainBusinessPopupButton.length > 0 ){
            mainBusinessPopupButton.on('click', function () {
                var parentConatiner = $(document).find(".ays-quiz-popup-box-main-conatiner");

                parentConatiner.css({'display': 'none'});

                if( !parentConatiner.hasClass('ays-quiz-popup-box-main-conatiner-type-chatgpt') ){
                    var cookieTypeName = parentConatiner.attr('data-type');

                    var if_cookie_exists = aysQuizGetCookie("ays_quiz_popup_box_" + cookieTypeName);
                    var if_weekly_cookie_exists = aysQuizGetCookie("ays_quiz_popup_box_weekly");

                    if ( typeof if_cookie_exists == "undefined" || if_cookie_exists == "" ) {
                        aysQuizCreateCookie("ays_quiz_popup_box_" + cookieTypeName, 1, 60 );
                    }

                    if ( typeof if_weekly_cookie_exists == "undefined" || if_weekly_cookie_exists == "" ) {
                        aysQuizCreateCookie("ays_quiz_popup_box_weekly", 1, 7 );
                    }
                }
            });
        }

        var topBannerBusinessPopupButton = $(document).find("div#ays-quiz-new-mega-bundle-dicount-month-main.ays_quiz_dicount_info button");
        if( topBannerBusinessPopupButton.length > 0 ){
            topBannerBusinessPopupButton.on('click', function () {
                var parentConatiner = $(document).find("#ays-quiz-new-mega-bundle-dicount-month-main");
        ...
        $(document).on('click', "button.ays-quiz-popup-box-ai-addon-icon", function () {
            var parentConatiner = $(document).find(".ays-quiz-popup-box-main-conatiner.ays-quiz-popup-box-main-conatiner-type-chatgpt");
            parentConatiner.css({'display': 'flex'});
        });
        // Quiz Business banner | End
        ...
        // Interval message | open popup editor start
        $(document).on('click', '.ays-quiz-open-intervals-editor', function(e){
            var _this = $(this);
            var editorPopup = $(document).find('#ays-open-intervals-editor');
            var containertd = _this.parents("td");
            var textarea = containertd.find('textarea');
            var intervalContent = textarea.val();

            textarea.attr('opened', 'opened');

            if ( $(document).find("#wp-ays_quiz_interval_text_editor-wrap").hasClass("tmce-active") ){
                window.tinyMCE.get('ays_quiz_interval_text_editor').setContent( intervalContent );
            }else{
                $(document).find('#ays_quiz_interval_text_editor').val( intervalContent );
            }

            // $(document).find('.mce-container.mce-panel.mce-floatpanel.mce-window.mce-in').addClass('mce-container-ays-modal');
            editorPopup.aysModal('open');
        });
        
        $(document).on('click', '.ays-quiz-apply-question-changes', function(e){
            var _this = $(this);
            var editorPopup = $(document).find('#ays-open-intervals-editor');
            var textarea = $(document).find("textarea[opened='opened']");
            textarea.removeAttr('opened');

            if ( $(document).find("#wp-ays_quiz_interval_text_editor-wrap").hasClass("tmce-active") ){
                var txt = window.tinyMCE.get('ays_quiz_interval_text_editor').getContent() ;
            }else{
                var txt =  $(document).find('#ays_quiz_interval_text_editor').val( );
            }
            textarea.val(txt);
        ...
        $(document).on('click', '.ays-close.ays-close-editor', function(e){
            var textarea = $(document).find("textarea[opened='opened']");
            textarea.removeAttr('opened');
        });
        $(document).on("keydown", function(e){
            if(e.keyCode === 27){
                var textarea = $(document).find("textarea.interval_text[opened='opened']");
                textarea.removeAttr('opened');
                return false;
            }
        });
        // Interval message | open popup editor end

        $(document).on('click', ".ays-quiz-interval-add-email-message-files", function (e) {
            openMediaUploaderForIntervals(e, $(this));
        });

        $(document).on('click', ".ays-quiz-interval-email-message-files-wrapper", function () {
            deleteCurrentEmailFile($(this));
        });
    });
