...
class Quiz_Maker_Admin
{
    ...
    public function enqueue_styles($hook_suffix){
        ...
        // wp_enqueue_style($this->plugin_name . '-pro-features', plugin_dir_url(__FILE__) . 'css/quiz-maker-pro-features.css', array(), time(), 'all');
    ...
    public function enqueue_scripts($hook_suffix){
        ...
        */
        ...
        wp_localize_script( $this->plugin_name, 'quizLangObj', array(
            ...
            "copied"                            => __( "Copied!", $this->plugin_name ),
            "clickForCopy"                      => __( "Click for copy", $this->plugin_name ),
            "fillInBlankTypeSlugText"           => __( "Please copy and paste the slugs instead of the missing words in the question textarea.", $this->plugin_name ),
            ...
            'thumbsUpGreat'                     => __( 'Thumbs up, great!', $this->plugin_name),
            'intervalAddFile'                   => __( 'Add file', $this->plugin_name ),
            'intervalEditFile'                  => __( 'Edit file', $this->plugin_name ),
            'intervalEmailAttachment'           => __( 'E-mail Attachment file', $this->plugin_name ),
            'intervalEmailAttachmentTooltip'    => __( 'Attach a file to the Interval Message. Please note, that the system will send the attachment via email only if the Send interval message to user and the Send Interval message to admin options are enabled for the quiz. If these options are enabled, the corresponding file attachment will be sent to the user when appearing in the particular Interval. Note: The feature will not work if the %%interval_message%% message variable is inserted in the Email Message option and if any of these options is disabled for the quiz.', $this->plugin_name ),
    ...
    public function add_plugin_admin_menu(){
        ...
        add_menu_page(
            ...
            AYS_QUIZ_ADMIN_URL . '/images/icons/quiz-maker-icon-logo.svg', 
            '6.20'
        );
    ...
    public function screen_option_quiz_attributes(){
        ...
        $args = array(
            ...
            'default' => 20,
            'option' => 'quiz_attributes_per_page'
        );
        ...
        if ( (isset($_REQUEST["action"]) && $_REQUEST["action"] == "add_question_rows") || (isset($_REQUEST["action"]) && $_REQUEST["action"] == "add_question_rows_top") ) {
            ...
            if (!empty($question_ids)) {
                ...
                foreach ($question_ids as $question_id) {
                    ...
                    $data_tag_id_str = isset( $data['tag_id'] ) && !empty( $data['tag_id'] ) ? $data['tag_id'] : "";

                    $tag_ids = explode(',',$data_tag_id_str);
                    $question_tags_title = '';
                    ...
                    $question_category_title = isset( $question_categories_array[$data['category_id']] ) && $question_categories_array[$data['category_id']] != "" ? esc_attr( $question_categories_array[$data['category_id']] ) : "";
                    ...
                    $rows[] = '<tr class="ays-question-row ui-state-default" data-id="' . $data['id'] . '">
                                    ...
                                    <td class="ays-quiz-question-category-row">' . $question_category_title . '</td>
	...
	public function ays_questions_export() {
        ...
        if(isset($questions) && !empty($questions)){
            ...
            foreach ($questions as $key => $question) {
                ...
                $new_question_id = isset($question['id']) && $question['id'] != "" ? absint($question['id']) : null;
                
                if( $new_question_id != "" && !is_null($new_question_id) ){
                    if( !in_array($new_question_id, $new_question_ids) ){
            ...
            if( isset($new_questions) && !empty($new_questions) ){
    ...
    public function ays_questions_export_simple_xlsx($questions) {
        ...
        foreach ( $questions as &$question ) {
            ...
            $category = (isset($question['category_title']) && $question['category_title'] != '') ? $question['category_title'] : 'Uncategorized';
            $tag_title = (isset($question['tag_title']) && $question['tag_title'] != '') ? $question['tag_title'] : '';
            ...
            array_splice($q,4,0,$correct_answer);
            $quests[] = $q;
        }
    ...
    public function ays_single_question_results_export() {
        ...
        if (isset($_REQUEST['action']) && $_REQUEST['action'] == 'ays_single_question_results_export') {
            ...
            $answers_keyword_counts = isset($options->answers_keyword_counts) ? (array)$options->answers_keyword_counts : array();
            ...
            if(isset( $answers_keyword_counts) &&  !empty($answers_keyword_counts)){
                ...
                foreach ($answers_keyword_counts as $key => $value) {
                    ...
                    $answers_keyword_count_arr[] = $key .' -> ' . $value . ' (' . (round($mv_keyword_percentage,2)). '%)';
                }

                if ( !empty($answers_keyword_count_arr) ) {
                    $answers_keyword_count_str = implode(", ", $answers_keyword_count_arr);
                }
            }

            $enable_html = false;
            $results_by_categories = Quiz_Maker_Data::ays_quiz_current_result_by_category($options, $question_correctness, $question_id_arr, $calc_method, $enable_html);

            $results_by_categories_str = "";
            if(isset( $results_by_categories) && !empty($results_by_categories)){
                $results_by_categories_str = trim(strip_tags($results_by_categories));
            }
            ...
            $quiz_information_headers = array(
                ...
                __( "Keywords", $this->plugin_name ),
                __( "Results by Categories", $this->plugin_name ),
            );
            ...
            foreach ($options->correctness as $key => $option) {
                if (strpos($key, 'question_id_') !== false) {
                    ...
                    if($this->question_is_text_type($question_id)){
                        ...
                        $user_answered = $this->get_user_matching_answered($options->user_answered, $key, $answer_incorrect_matches, $question_id);
    ...
    public function ays_export_result_pdf() {
        ...
        if (isset($_REQUEST['action']) && $_REQUEST['action'] == 'ays_export_result_pdf') {
            ...
            $data_headers['user_data'] = array(
                ...
                'api_keywords_header'           =>  __( "Keywords", $this->plugin_name ),
                'api_res_by_cats_header'        =>  __( "Results by Categories", $this->plugin_name ),
                ...
                'api_keywords'                  =>  $answers_keyword_count_str,
                'api_res_by_cats'               =>  $results_by_categories_str,
            );
                ...
                if (strpos($key, 'question_id_') !== false) {
                    ...
                    if( $question_type == "fill_in_blank" ){
                        ...
                        foreach ($answers_array as $answer_key => $answer_data) {
                            ...
                            $answers_status_class = 'margin:0;color: red;';
                            if(mb_strtolower(trim($user_answer)) == mb_strtolower(trim($corect_answer))){
                                $answers_status_class = 'margin:0;color: green;';
                                $answer_html = '<span style="'. $answers_status_class .'">' . $user_answer . '</span>';
                            } elseif( $user_answer == "" ){
                                $answer_html = '<span style="'. $answers_status_class .'">' . "-" . '</span>';
                            } else {
                                $answer_html = '<span style="'. $answers_status_class .'">' . $user_answer . '</span>';
                            }
                    ...
                    if( $is_matching_type ){
                        ...
                        foreach ( $correct_answers as $key => $_correct_answer ) {
                            ...
                            $answers_status_class = 'margin:0;color: red;';
                            if( isset($user_answered[$key]['correct']) && $user_answered[$key]['correct'] ){
                                $answers_status_class = 'margin:0;color: green;';
                            }
                            ...
                            $correct_answer      .= html_entity_decode( ( stripslashes( $_correct_answer ) ) ) . $separator;
                            $user_answer         .= '<span style="'. $answers_status_class .'">' . html_entity_decode( stripslashes( $user_answered[$key]['answer'] ) ) . '</span>' . $separator;
                        ...
                        $correct_answer = html_entity_decode(strip_tags(stripslashes($correct_answers)));
                        if( $question_type == "fill_in_blank" ){
                            $user_answer    = html_entity_decode(stripslashes($user_answered));
                        } else {
                            $user_answer    = html_entity_decode(strip_tags(stripslashes($user_answered)));
                        }
                        $questions = array(
    ...
    public function ays_quick_start(){
        ...
        $answers_correct = Quiz_Maker_Data::recursive_sanitize_text_field( $_REQUEST['ays_quick_answer_correct'] );
        $answers = Quiz_Maker_Data::recursive_sanitize_text_field( $_REQUEST['ays_quick_answer'] );
        ...
        $options = json_encode(array(
            ...
            'quiz_question_text_to_speech_voice'        => 'Microsoft David - English (United States)',
        ...
        if (isset($_REQUEST['action']) && $_REQUEST['action'] == 'ays_show_results') {
            ...
            if ($ays_quiz_show_result_info_user_id == 'on') {
                if($user_id !== 0){
                    $row .= '<tr class="ays_result_element">
                            ...
                            <td colspan="3">' . esc_attr($user_id) . '</td>
                        </tr>';
                }
            }
            ...
            if ($user_attributes !== null) {
                ...
                foreach ($user_attributes as $name => $value) {
                    ...
                    if ( preg_match( "/^(http[s]?:\\/\\/)/", $attr_value ) ) {
                        $file_name = wp_basename($attr_value);
                        $file_attr_value = '<div class="ays-survey-answer-upload-ready">';
                            $file_attr_value .= '<a href="'.esc_url($attr_value).'" target="_blank" class="ays-survey-answer-upload-ready-link">'.$file_name.'</a>';
                        $file_attr_value .= '</div>';
                        $attr_value = $file_attr_value;
                    } else {
                        $attr_value = esc_attr($attr_value);
                    }
                    
                    $row .= '<tr class="ays_result_element">
                            <td>' . esc_attr(stripslashes($name)) . '</td>
                ...
                if (strpos($key, 'question_id_') !== false) {
                    ...
                    if($calc_method == 'by_correctness'){
                        ...
                        if($is_text_type && ! in_array($text_type, $not_multiple_text_types)){
                            ...
                            $row .= '<p class="success">' . nl2br( stripslashes( esc_attr( $c_answer ) ) ) . '<br>'.$correct_answer_images.'</p>';
                            $row .= '</td>';
                        }elseif( $question_type == "fill_in_blank" ){
                        ...
                        if( $question_type == "fill_in_blank" ){
                            ...
                            $user_answer_content = nl2br( stripslashes( esc_attr( $user_answered ) ) );
                            if($use_html){
                                $user_answer_content = stripslashes( $user_answered );
                            }
                        ...
                        if( $question_type == "fill_in_blank" ){
                            ...
                            $row .= '<td class="ays-report-user-answer ays-report-user-answer-by-points">'.__('User answered',$this->plugin_name).':<br/><p class="'.$ans_point_class.'">' . nl2br(esc_attr(stripslashes($user_answered))) . '</p></td>
                                    <td class="ays-report-answer-point">'.__('Answer point',$this->plugin_name).':<br/><p class="'.$ans_point_class.'">' . esc_attr($ans_point) . '</p></td>
                                </tr>';
                        }
                    }
    ...
    public function get_user_matching_answered($user_choice, $key, $incorrect_matches, $question_id){
        global $wpdb;
        $answers_table = $wpdb->prefix . "aysquiz_answers";
        $choices = $user_choice->$key;
        $text = array();
    ...
    public function get_published_questions_ajax(){
        ...
        foreach ($questions as $index => $question) {
            ...
            $first_column = '<span>';
            if (isset($question_id_array) && in_array($question["id"], $question_id_array)){
            ...
            if( isset( $question['type'] ) && $question['type'] != "" ){
                ...
                $ays_question_type = 'radio';
            }
            
            $results[] = array(
                'first_column'  => $first_column,
                'id'            => $question['id'],
                'type'          => $ays_question_type,
    ...
    public function ays_quiz_author_user_search() {
        ...
        // Run a security check.
        check_ajax_referer( 'quiz-maker-ajax-author-user-search-nonce', sanitize_key( $_REQUEST['_ajax_nonce'] ) );
    ...
    public function ays_quiz_business_popup_box_banner(){
        $if_weekly_cookie_exists = Quiz_Maker_Data::ays_quiz_global_check_cookie('ays_quiz_popup_box_weekly');
        if( current_user_can( 'manage_options' ) && !$if_weekly_cookie_exists ){
            global $wpdb;
            ...
            // woocommerce
            if( !isset( $_COOKIE['ays_quiz_popup_box_woocommerce'] ) ){
            ...
            if( !isset( $_COOKIE['ays_quiz_popup_box_edd'] ) ){
                if( in_array('easy-digital-downloads/easy-digital-downloads.php', apply_filters('active_plugins', get_option('active_plugins'))) 
                 || in_array('easy-digital-downloads-pro/easy-digital-downloads.php', apply_filters('active_plugins', get_option('active_plugins'))) ) {
                    if( !in_array('quiz-maker-edd/quiz-maker-edd.php', apply_filters('active_plugins', get_option('active_plugins'))) ) {
                        $type = 'edd';
                    }
                }
            }
            ...
            // Chained Quiz Addon
            if( !isset( $_COOKIE['ays_quiz_popup_box_chained_quiz'] ) ){
                if( !in_array('quiz-maker-chained-quizzes/quiz-maker-chained-quizzes.php', apply_filters('active_plugins', get_option('active_plugins'))) ) {
            ...
            // ReCaptcha
            if( !isset( $_COOKIE['ays_quiz_popup_box_recaptcha'] ) ){
                if( isset($_GET["page"]) && sanitize_key($_GET["page"]) != ""){
                    $if_plugin_screen_page  = strpos( sanitize_key( $_GET["page"] ) , AYS_QUIZ_NAME . '-results');
                    $if_plugin_screen_page2 = strpos( sanitize_key( $_GET["page"] ) , AYS_QUIZ_NAME . '-each-result');
                    ...
                    if( $if_plugin_screen_page2 !== false && ( $if_plugin_screen_page2 === 0 || $if_plugin_screen_page2 >= 0) ){
                        $results_count = $wpdb->get_var($sql);
                        if( $results_count >= 1000 ){
                            $type = 'recaptcha';
                        }
            ...
            if( $type != "" ){
                $content = $this->ays_quiz_footer_popup_box_banner($type);
            }

            if( !empty($content) && $content != "" ){
                // Quiz_Maker_Data::ays_quiz_global_set_cookie('ays_quiz_popup_box_weekly', 1, 7);
                echo $content;
            }
        }
    ...
    public function ays_quiz_footer_popup_box_banner( $type ) {
        ...
        switch ( $type ) {
            case 'woocommerce':
                $popup_title = "Integrate <strong>WooCommerce</strong> with <strong>Quiz Maker</strong> Plugin!";
                $popup_desc  = "Turn quizzes into cash! Sell quizzes as WooCommerce products on your WordPress site effortlessly.";
                $popup_icon_img  = AYS_QUIZ_ADMIN_URL . "/images/icons/addon-woocommerce.png";
                $target_link = "https://ays-pro.com/woocommerce-integration-addon?utm_source=dashboard&utm_medium=quiz-business&utm_campaign=woo-addon";
                break;
            case 'edd':
                $popup_title = "Integrate <strong>Easy Digital Downloads</strong> with <strong>Quiz Maker</strong> Plugin!";
                $popup_desc  = "Turn quizzes into cash! Sell quizzes as Easy Digital Downloads products on your WordPress site effortlessly.";
                $popup_icon_img  = AYS_QUIZ_ADMIN_URL . "/images/icons/addon-edd.png";
                $target_link = "https://ays-pro.com/easy-digital-downloads-addon?utm_source=dashboard&utm_medium=quiz-business&utm_campaign=edd-addon";
                break;
            case 'chained_quiz':
                $popup_title = "Chained Quiz Addon";
                $popup_desc  = "Get <strong>Chained Quiz</strong> Addon and effortlessly connect over 20 quizzes and let your users pass them one by one!";
                $popup_icon_img  = AYS_QUIZ_ADMIN_URL . "/images/icons/addon-chained-quiz.png";
                $target_link = "https://ays-pro.com/chained-quiz-addon?utm_source=dashboard&utm_medium=quiz-business&utm_campaign=chained-quiz-addon";
                break;
            case 'recaptcha':
                $popup_title = "Level Up Your Security!";
                $popup_desc  = "Upgrade to the Developer Version for having <strong>Recapcha</strong> Integration. With over 1000 quiz results, it's time for enhanced protection against spammers!";
                $popup_icon_img  = AYS_QUIZ_ADMIN_URL . "/images/icons/google-recaptcha-icon.svg";
                $target_link = "https://ays-pro.com/wordpress/quiz-maker?utm_source=dashboard&utm_medium=quiz-business&utm_campaign=recaptcha-popup-banner";
                break;
            case 'chatgpt':
                $popup_title = "Integrate <strong>ChatGPT</strong> with <strong>Quiz Maker</strong> Plugin!";
                $popup_desc  = "Oh, you have already created over 200 questions! Get ChatGPT addon and generate questions on any topic in seconds!";
                $popup_icon_img  = AYS_QUIZ_ADMIN_URL . "/images/icons/addon-ChatGPT-Question-Generator.png";
                $target_link = "https://ays-pro.com/chatgpt-integration-addon?utm_source=dashboard&utm_medium=quiz-business&utm_campaign=chatgpt-addon";
                break;
        ...
        $content[] = '<div class="ays-quiz-popup-box-main-conatiner ays-quiz-popup-box-main-conatiner-type-'. $type .'" data-type="'. $type .'">';
            $content[] = '<div class="ays-quiz-popup-box-conatiner">';
                    ...
                    if( $popup_icon_img != "" ){
                        $content[] = '<div class="ays-quiz-popup-box-header-img-row">';
                            $content[] = '<div class="ays-quiz-popup-box-header-img">';
                                $content[] = '<img src="'.  $popup_icon_img . '">';
                            $content[] = '</div>';
                        $content[] = '</div>';
                    }
                ...
                $content[] = '<div class="ays-quiz-popup-box-action">';
                    $content[] = '<div class="ays-quiz-popup-box-action-button-row">';
                        $content[] = '<a href="'. $target_link .'" target="blank" class="ays-quiz-popup-box-action-button">';
                            $content[] = "Try it now";
                        $content[] = '</a>';
                    $content[] = '</div>';
                $content[] = '</div>';
    ...
    public function ays_quiz_business_popup_box_top_banner(){
        $if_weekly_cookie_exists = Quiz_Maker_Data::ays_quiz_global_check_cookie('ays_quiz_popup_box_weekly');
        if( current_user_can( 'manage_options' ) && !$if_weekly_cookie_exists ){
            global $wpdb;
            ...
            // gamipress
            if( !isset( $_COOKIE['ays_quiz_popup_box_gamipress'] ) ){
                if( in_array('gamipress/gamipress.php', apply_filters('active_plugins', get_option('active_plugins'))) ) {
                    if( !in_array('quiz-maker-gamipress/quiz-maker-gamipress.php', apply_filters('active_plugins', get_option('active_plugins'))) ) {
                        $type = 'gamipress';
                    }
                }
            }
            ...
            // mycred
            if( !isset( $_COOKIE['ays_quiz_popup_box_mycred'] ) ){
                if( in_array('mycred/mycred.php', apply_filters('active_plugins', get_option('active_plugins'))) ) {
                    if( !in_array('quiz-maker-mycred/quiz-maker-mycred.php', apply_filters('active_plugins', get_option('active_plugins'))) ) {
                        $type = 'mycred';
                    }
                }
            }

            // user_dashboard_track_users
            if( !isset( $_COOKIE['ays_quiz_popup_box_user_dashboard_track_users'] ) ){
                if( isset($_GET["page"]) && sanitize_key($_GET["page"]) != ""){
                    $if_plugin_screen_page  = strpos( sanitize_key( $_GET["page"] ) , AYS_QUIZ_NAME . '-settings');
                    if( $if_plugin_screen_page !== false && ( $if_plugin_screen_page === 0 || $if_plugin_screen_page >= 0) ){
                        if( !in_array('quiz-maker-track-users/quiz-maker-track-users.php', apply_filters('active_plugins', get_option('active_plugins'))) 
                         || !in_array('quiz-maker-user-dashboard/quiz-maker-user-dashboard.php', apply_filters('active_plugins', get_option('active_plugins'))) ) {
                            // $wp_users_count = get_user_count();
                            $db_prefix = is_multisite() ? $wpdb->base_prefix : $wpdb->prefix;
                            $sql = "SELECT COUNT(*) FROM {$db_prefix}users";
                            $wp_users_count = $wpdb->get_var($sql);
                            if( isset($wp_users_count) && intval($wp_users_count) >= 50 ){
                                $type = 'user_dashboard_track_users';
                            }
                        }
                    }
            ...
            // summary_emails
            if( !isset( $_COOKIE['ays_quiz_popup_box_summary_emails'] ) ){
                if( !in_array('mycred/mycred.php', apply_filters('active_plugins', get_option('active_plugins'))) ) {
                    $sql = "SELECT COUNT(*) FROM {$wpdb->prefix}aysquiz_reports";
                    $results_count = $wpdb->get_var($sql);
                    if( $results_count >= 1000 ){
            ...
            // integrations
            if( !isset( $_COOKIE['ays_quiz_popup_box_integrations'] ) ){
                if( isset($_GET["page"]) && sanitize_key($_GET["page"]) != ""){
                    $if_plugin_screen_page  = strpos( sanitize_key( $_GET["page"] ) , AYS_QUIZ_NAME . '-settings');
                    if( $if_plugin_screen_page !== false && ( $if_plugin_screen_page === 0 || $if_plugin_screen_page >= 0) ){
                        $type = 'integrations';
            ...
            if( $type != "" ){
                $content = $this->ays_quiz_popup_box_top_banner_message($type);
            }
    ...
    // New Mega Bundle
    public function ays_quiz_popup_box_top_banner_message($type){
        ...
        switch ( $type ) {
            case 'gamipress':
                $target_link = "https://ays-pro.com/gamipress-integration-addon?utm_source=dashboard&utm_medium=quiz-business&utm_campaign=gamipress-addon";
                $popup_title = "Integrate <a href='". $target_link ."' target='_blank' class='ays-quiz-sale-banner-link'><strong>Gamipress</strong></a> with <strong>Quiz Maker</strong> Plugin!";
                $popup_desc  = "Level up user engagement and boost site traffic by rewarding users with points for taking your quizzes.";
                $popup_icon_img  = AYS_QUIZ_ADMIN_URL . "/images/icons/addon-gamipress.png";
                break;
            case 'mycred':
                $target_link = "https://ays-pro.com/mycred-integration-addon?utm_source=dashboard&utm_medium=quiz-business&utm_campaign=mycred-addon";
                $popup_title = "Integrate <a href='". $target_link ."' target='_blank' class='ays-quiz-sale-banner-link'><strong>MyCred</strong></a> with <strong>Quiz Maker</strong> Plugin!";
                $popup_desc  = "Level up user engagement and boost site traffic by rewarding users with points for taking your quizzes.";
                $popup_icon_img  = AYS_QUIZ_ADMIN_URL . "/images/icons/addon-mycred.png";
                break;
            case 'user_dashboard_track_users':
                $target_link    = "https://ays-pro.com/user-dashboard-addon?utm_source=dashboard&utm_medium=quiz-business&utm_campaign=user-dashboard-addon";
                $target_link2   = "https://ays-pro.com/track-users-addon?utm_source=dashboard&utm_medium=quiz-business&utm_campaign=track-users-addon";
                $popup_title    = "Enhance Your Quiz Analytics! ";
                $popup_desc     = "Unlock the <a href='". $target_link ."' target='_blank' class='ays-quiz-sale-banner-link'><strong>User Dashboard</strong></a> and <a href='". $target_link2 ."' target='_blank' class='ays-quiz-sale-banner-link'><strong>Track Users</strong></a> Addons for full tracking. Monitor daily activity, track tab changes, hint usage, and many more.";
                $popup_icon_img = AYS_QUIZ_ADMIN_URL . "/images/icons/addon-user-dashboard.png";
                break;
            case 'summary_emails':
                $target_link = "https://ays-pro.com/summary-emails-add-on?utm_source=dashboard&utm_medium=quiz-business&utm_campaign=summary-emails-addon";
                $popup_title = "Stay Informed and Organized!";
                $popup_desc  = "Get detailed reports for quizzes on your preferred schedule (hourly, daily, weekly, or monthly) via email. Unlock <a href='". $target_link ."' target='_blank' class='ays-quiz-sale-banner-link'><strong>Summary Emails</strong></a> addon for better control and insights!";
                $popup_icon_img  = AYS_QUIZ_ADMIN_URL . "/images/icons/addon-summary-emails.png";
                break;
            case 'integrations':
                $target_link    = "https://ays-pro.com/addons-for-quiz-maker?utm_source=dashboard&utm_medium=quiz-business&utm_campaign=integration-banner";
                $target_link1   = "https://ays-pro.com/mailpoet-integration-addon?utm_source=dashboard&utm_medium=quiz-business&utm_campaign=mailpoet-integration-addon";
                $target_link2   = "https://ays-pro.com/klaviyo-integration-addon?utm_source=dashboard&utm_medium=quiz-business&utm_campaign=klaviyo-integration-addon";
                $target_link3   = "https://ays-pro.com/sendinblue-addon?utm_source=dashboard&utm_medium=quiz-business&utm_campaign=brevo-addon";
                $target_link4   = "https://ays-pro.com/mailerlite-addon?utm_source=dashboard&utm_medium=quiz-business&utm_campaign=mailerlite-addon";
                $target_link5   = "https://ays-pro.com/aweber-integration-addon?utm_source=dashboard&utm_medium=quiz-business&utm_campaign=aweber-integration-addon";
                $popup_title    = "";
                $popup_desc     = "Automatically gather user email addresses and seamlessly integrate with top email marketing tools like <a href='". $target_link2 ."' target='_blank' class='ays-quiz-sale-banner-link'><strong>Klaviyo</strong></a>, <a href='". $target_link5 ."' target='_blank' class='ays-quiz-sale-banner-link'><strong>Aweber</strong></a>, <a href='". $target_link3 ."' target='_blank' class='ays-quiz-sale-banner-link'><strong>Brevo</strong></a>, <a href='". $target_link4 ."' target='_blank' class='ays-quiz-sale-banner-link'><strong>MailerLite</strong></a>, and <a href='". $target_link1 ."' target='_blank' class='ays-quiz-sale-banner-link'><strong>MailPoet</strong></a>.";
                $popup_icon_img = AYS_QUIZ_ADMIN_URL . "/images/icons/addon-mailpoet.png";
                break;
            default:
                break;
        }
        ...
        if($type != ""){
            ...
            $content[] = '<div id="ays-quiz-new-mega-bundle-dicount-month-main" class="notice notice-success is-dismissible ays_quiz_dicount_info" data-type="'. $type .'">';
                $content[] = '<div id="ays-quiz-dicount-month" class="ays_quiz_dicount_month">';
                    ...
                    $content[] = '<div class="ays-quiz-dicount-wrap-box ays-quiz-dicount-wrap-text-box">';
                        ...
                        $content[] = '<div class="ays-quiz-dicount-wrap-logo-row">';
                            $content[] = '<div class="ays-quiz-dicount-wrap-logo-min-box">';
                                $content[] = '<a href="'. $target_link .'" target="_blank" class="ays-quiz-sale-banner-link"><img src="'. $popup_icon_img .'"></a>';
                            $content[] = '</div>';
                        $content[] = '</div>';

                        $content[] = '<div class="ays-quiz-dicount-wrap-text-row">';
                            $content[] = '<div class="ays-quiz-new-mega-bundle-title">';
                                $content[] = '<div class="ays-quiz-new-mega-bundle-title-heading">';
                                    $content[] = $popup_title;
                                $content[] = '</div>';
                                $content[] = '<div class="ays-quiz-new-mega-bundle-desc">';
                                    $content[] = $popup_desc;
                                $content[] = '</div>';
                            $content[] = '</div>';
                        $content[] = '</div>';
                    ...
                    $content[] = '<div class="ays-quiz-dicount-wrap-box ays-quiz-dicount-wrap-button-box">';
                        $content[] = '<a href="'. $target_link .'" class="button button-primary ays-button" id="ays-button-top-buy-now" target="_blank">' . 'Get the Addon' . '</a>';
                    $content[] = '</div>';
                $content[] = '</div>';
            $content[] = '</div>';
