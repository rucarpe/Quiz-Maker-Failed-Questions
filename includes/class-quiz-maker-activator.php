...
class Quiz_Maker_Activator {
    ...
    public static function ays_quiz_update_db_check() {
        ...
        if(is_multisite()){
            ...
            if($wpdb->get_var("SHOW TABLES LIKE '$quizes_table'") != $quizes_table) {
                delete_network_option($network_id, 'ays_quiz_db_version');
            }

            if($wpdb->get_var("SHOW TABLES LIKE '$question_tags_table'") != $question_tags_table) {
                delete_network_option($network_id, 'ays_quiz_db_version');
            }

            if ( get_network_option( $network_id, 'ays_quiz_db_version' ) != $ays_quiz_db_version ) {
                self::activate();
