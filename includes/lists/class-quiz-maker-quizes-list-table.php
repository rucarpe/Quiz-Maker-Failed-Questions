...
class Quizes_List_Table extends WP_List_Table{
    ...
    public function add_or_edit_quizes($data){
        ...
        if( isset($data["quiz_action"]) && wp_verify_nonce( $data["quiz_action"],'quiz_action' ) ){
            ...
            */
            ...
            $quiz_question_text_to_speech_voice = (isset( $_POST[ 'ays_quiz_question_text_to_speech_voice' ] ) && $_POST[ 'ays_quiz_question_text_to_speech_voice' ] != '') ? sanitize_text_field($_POST[ 'ays_quiz_question_text_to_speech_voice' ]) : '';
            ...
            $quiz_custom_texts_send_feedback_button = (isset($_REQUEST['ays_quiz_custom_texts_send_feedback_button']) && $_REQUEST['ays_quiz_custom_texts_send_feedback_button'] != '') ? stripslashes( sanitize_text_field( $_REQUEST['ays_quiz_custom_texts_send_feedback_button'] ) ) : $gen_send_feedback_button;

            // Load more button
            $quiz_custom_texts_load_more_button = (isset($_REQUEST['ays_quiz_custom_texts_load_more_button']) && $_REQUEST['ays_quiz_custom_texts_load_more_button'] != '') ? stripslashes( sanitize_text_field( $_REQUEST['ays_quiz_custom_texts_load_more_button'] ) ) : $gen_load_more_button;

            // Exit button
            $quiz_custom_texts_exit_button = (isset($_REQUEST['ays_quiz_custom_texts_exit_button']) && $_REQUEST['ays_quiz_custom_texts_exit_button'] != '') ? stripslashes( sanitize_text_field( $_REQUEST['ays_quiz_custom_texts_exit_button'] ) ) : $gen_exit_button;

            // Check button
            $quiz_custom_texts_check_button = (isset($_REQUEST['ays_quiz_custom_texts_check_button']) && $_REQUEST['ays_quiz_custom_texts_check_button'] != '') ? stripslashes( sanitize_text_field( $_REQUEST['ays_quiz_custom_texts_check_button'] ) ) : $gen_check_button;

            // Login button
            $quiz_custom_texts_login_button = (isset($_REQUEST['ays_quiz_custom_texts_login_button']) && $_REQUEST['ays_quiz_custom_texts_login_button'] != '') ? stripslashes( sanitize_text_field( $_REQUEST['ays_quiz_custom_texts_login_button'] ) ) : $gen_login_button;

            // Show quiz category description
            $quiz_enable_quiz_category_description = ( isset($_REQUEST['ays_quiz_enable_quiz_category_description']) && sanitize_text_field( $_REQUEST['ays_quiz_enable_quiz_category_description'] ) == 'on' ) ? 'on' : 'off';
            ...
            $quiz_navigation_bar_type = (isset($_REQUEST['ays_quiz_navigation_bar_type']) && sanitize_text_field( $_REQUEST['ays_quiz_navigation_bar_type']) != '') ? sanitize_text_field( $_REQUEST['ays_quiz_navigation_bar_type'] ) : 'default';

            // Show question tags
            $quiz_show_question_tags = (isset($_REQUEST['ays_quiz_show_question_tags']) && $_REQUEST['ays_quiz_show_question_tags'] == "on") ? 'on' : 'off';

            // Show/hide results based on pass score
            $quiz_show_results_based_pass_score = (isset($_REQUEST['ays_quiz_show_results_based_pass_score']) && $_REQUEST['ays_quiz_show_results_based_pass_score'] == "on") ? 'on' : 'off';

            // Show Interval Messages for equal keywords
            $quiz_show_interval_messages_for_equal_keywords = (isset($_REQUEST['ays_quiz_show_interval_messages_for_equal_keywords']) && $_REQUEST['ays_quiz_show_interval_messages_for_equal_keywords'] == "on") ? 'on' : 'off';

            // Hide the Banner Question type
            $quiz_hide_banner_questions_on_result_page = (isset($_REQUEST['ays_quiz_hide_banner_questions_on_result_page']) && $_REQUEST['ays_quiz_hide_banner_questions_on_result_page'] == "on") ? 'on' : 'off';

            // Answer view grid count
            $quiz_grid_view_count = (isset($_REQUEST[ 'ays_quiz_grid_view_count' ]) && $_REQUEST[ 'ays_quiz_grid_view_count' ] != '') ? stripslashes( sanitize_text_field( $_REQUEST[ 'ays_quiz_grid_view_count' ] ) ) : '2';
            
            // Answer view grid only for image
            $quiz_grid_image_view = (isset($_REQUEST[ 'ays_quiz_grid_image_view' ]) && $_REQUEST[ 'ays_quiz_grid_image_view' ] == "on") ? 'on' : 'off';

            // Send results without images User
            $quiz_send_results_user_without_img = (isset($_REQUEST[ 'ays_quiz_send_results_user_without_img' ]) && $_REQUEST[ 'ays_quiz_send_results_user_without_img' ] == "on") ? 'on' : 'off';

            // Send results without images Admin
            $quiz_send_results_admin_without_img = (isset($_REQUEST[ 'ays_quiz_send_results_admin_without_img' ]) && $_REQUEST[ 'ays_quiz_send_results_admin_without_img' ] == "on") ? 'on' : 'off';
            ...
            $options = array(
                ...
                'quiz_grid_view_count'                              => $quiz_grid_view_count,
                ...
                'quiz_navigation_bar_type'                          => $quiz_navigation_bar_type,
                'quiz_show_question_tags'                           => $quiz_show_question_tags,
                'quiz_show_results_based_pass_score'                => $quiz_show_results_based_pass_score,
                'quiz_show_interval_messages_for_equal_keywords'    => $quiz_show_interval_messages_for_equal_keywords,
                'quiz_hide_banner_questions_on_result_page'         => $quiz_hide_banner_questions_on_result_page,
            ...
            if( isset( $data['ays_quiz_timer'] ) && $data['ays_quiz_timer'] != 0 && $data['ays_quiz_timer'] != "" ){
                $options['timer'] = absint( sanitize_text_field( $data['ays_quiz_timer'] ) );
            }else{
                $options['timer'] = 100;
            }
            ...
            $interval_file = isset($_REQUEST['interval_messages_email_file']) && !empty( $_REQUEST['interval_messages_email_file'] ) ? $_REQUEST['interval_messages_email_file'] : array();
            $interval_file_id = isset($_REQUEST['interval_messages_email_file_id']) && !empty( $_REQUEST['interval_messages_email_file_id'] ) ? $_REQUEST['interval_messages_email_file_id'] : array();
            $interval_file_name = isset($_REQUEST['interval_messages_email_file_name']) && !empty( $_REQUEST['interval_messages_email_file_name'] ) ? $_REQUEST['interval_messages_email_file_name'] : array();
            $intervals = array();
            if( isset( $intervals_max ) && !is_null( $intervals_max ) && !empty( $intervals_max ) ){
                for($i = 0; $i < count($intervals_max); $i++){
                    ...
                    $intervals[$i]['interval_file'] = isset( $interval_file[$i] ) && $interval_file[$i] != "" ? sanitize_url($interval_file[$i]) : "";
                    $intervals[$i]['interval_file_id'] = isset( $interval_file_id[$i] ) && $interval_file_id[$i] != "" ? sanitize_text_field($interval_file_id[$i]) : "";
                    $intervals[$i]['interval_file_name'] = isset( $interval_file_name[$i] ) && $interval_file_name[$i] != "" ? sanitize_text_field($interval_file_name[$i]) : "";
            ...
            if($message == 'created'){
                setcookie('ays_quiz_created_new', $quiz_id, time() + 3600, '/');
            }
    ...
    public function duplicate_quizzes( $id ){
        ...
        $options['enable_google_sheets'] = "off";
