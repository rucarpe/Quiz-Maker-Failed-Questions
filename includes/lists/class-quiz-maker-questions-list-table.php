...
class Questions_List_Table extends WP_List_Table{
    ...
    public function get_question_max_id() {
        global $wpdb;
        $question_table = $wpdb->prefix . 'aysquiz_questions';

        $sql = "SELECT max(id) FROM {$question_table}";
    ...
    public function get_question_answer_max_id() {
        global $wpdb;
        $answers_table = $wpdb->prefix . 'aysquiz_answers';

        $sql = "SELECT max(id) FROM {$answers_table}";
    ...
    public function add_edit_questions($data){
        ...
        if( isset($data["question_action"]) && wp_verify_nonce( $data["question_action"],'question_action' ) ){
            ...
            if ( isset( $_POST['ays_question'] ) ) {
                $question = preg_replace("#<script(.*?)>(.*?)</script>#is", "", $_POST['ays_question']);
            ...
            foreach ( $answer_incorrect_matches_arr as $vkey => $value ) {
                $answer_incorrect_matches[ -($vkey +1) ] = stripslashes($value);
            }
	...
	public function questions_import( $import_file ) {
        ...
        $question_db_max_id         = $this->get_question_max_id();
        $question_answer_db_max_id  = $this->get_question_answer_max_id();
        ...
        foreach($for_import as $key => $question){
            ...
            if( isset( $question_db_max_id ) && isset($db_question_id) && !empty($db_question_id) && $question_db_max_id < $db_question_id ){
                $db_question_id = "";
            }
            ...
            foreach ( $question['answers'] as &$answer ) {
                ...
                if( isset( $question_answer_db_max_id ) && isset($db_answer_id) && !empty($db_answer_id) && $question_answer_db_max_id < $db_answer_id ){
                    $db_answer_id = "";
                }
            ...
            if( isset( $question['type'] ) && $question['type'] == "custom" && $quest_res >= 0 ){
                $imported++;
            }elseif($quest_res >= 0 && $answer_res_success > 0){
                $imported++;
            }else{
                $failed++;
            }
    ...
    public function ays_xlsx_questions_simple_import( $import_file_simple ){
        ...
        $tags_r = $wpdb->get_results("SELECT id, title FROM ".$question_tag_table, 'ARRAY_A');
        $tags = array();
        foreach($tags_r as $__tag){
            $tags[$__tag['id']] = strtolower($__tag['title']);
        }
        $ver = (float) phpversion();
        $imported = 0;
        $failed = 0;
        if ($ver >= 5.6) {
            ...
            foreach ($sheet_data as $sheet_value) {
                ...
                foreach ($sheet_value as $sh_key => $sh_val) {
                    switch ($sh_key) {
                        ...
                        case 'B':
                            ...
                            break;
                        case 'C':
                            $q_tag = htmlspecialchars_decode( preg_replace('/_x([0-9a-fA-F]{4})_/', '&#x$1;', $sh_val), ENT_HTML5);
                            $question_tags_str = htmlspecialchars_decode($q_tag, ENT_HTML5);
                            break;
                        case 'D':
                ...
                $question_tag_id = "";
                if( !empty( $question_tags_str ) ){
                    $question_tags = explode(",", $question_tags_str);
                    if (!empty( $question_tags )) {

                        $question_tag_id_arr = array();
                        foreach ($question_tags as $_key => $question_tag_title) {
                            // Insert Tag
                            if( is_null( $question_tag_title ) || empty($question_tag_title) ){
                                continue;
                            }

                            $question_tag_file = '';
                            if(isset($question_tag_title)){
                                $question_tag_file = trim( strval($question_tag_title) );
                            }
                            
                            if(in_array(strtolower($question_tag_file), $tags)){
                                $tag_id = array_search(strtolower($question_tag_file), $tags);
                                if($tag_id !== false){
                                    $question_tag_id_arr[] = intval($tag_id);
                                }else{
                                    continue;
                                }
                            }else{
                                $wpdb->insert(
                                    $question_tag_table,
                                    array(
                                        'title'  =>  $question_tag_file,
                                        'status'  =>  "published"
                                    ),
                                    array( '%s', '%s' )
                                );
                                $question_inerted_tag_id = $wpdb->insert_id;


                                $tags[$question_inerted_tag_id] = strtolower($question_tag_file);
                                $question_tag_id_arr[] = $question_inerted_tag_id;
                            }
                        }

                        if( !empty( $question_tag_id_arr ) ){
                            $question_tag_id = implode(",", $question_tag_id_arr);
                        }
    ...
    public function duplicate_question($id){
        ...
        $question_tag_ids = isset($questionDup['tag_id']) && $questionDup['tag_id'] != '' ? stripslashes(esc_attr($questionDup['tag_id'])) : "";
    ...
    public static function record_count() {
        ...
        if( $search ){
            $where[] = sprintf(" ( question LIKE '%%%s%%' OR question_title LIKE '%%%s%%' ) ", esc_sql( $wpdb->esc_like( $search ) ) , esc_sql( $wpdb->esc_like( $search ) )  );
        }
    ...
    function column_question( $item ) {
        ...
        if( $item['published'] == 2 ) {
            ...
            if( $owner ){
                $actions['edit'] = sprintf( '<a href="%s">'. __('Edit', $this->plugin_name) .'</a>', esc_url($url) );
            }else{
                $actions['edit'] = sprintf( '<a href="%s">'. __('View', $this->plugin_name) .'</a>', esc_url($url) );
            }

            $url_args['action']   = "duplicate";
            $url                  = add_query_arg( $url_args, $url );
            $actions['duplicate'] = sprintf( '<a href="%s">' . __( 'Duplicate', $this->plugin_name ) . '</a>', esc_url($url) );

            if ( $owner ) {
                $url_args['action']   = "trash";
                $url_args['_wpnonce'] = $trash_nonce;
                $url                  = add_query_arg( $url_args, $url );
                $actions['trash']     = sprintf( '<a href="%s">' . __( 'Move to trash', $this->plugin_name ) . '</a>', esc_url($url) );
            }
        }
