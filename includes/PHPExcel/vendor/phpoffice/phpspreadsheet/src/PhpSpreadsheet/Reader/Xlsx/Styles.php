...
class Styles extends BaseParserClass
{
    ...
    {
        $alignment->setHorizontal( (isset( $alignmentXml->alignment['horizontal'] ) ? (string) $alignmentXml->alignment['horizontal'] : 'general') );
        $alignment->setVertical( (isset( $alignmentXml->alignment['vertical'] ) ? (string) $alignmentXml->alignment['vertical'] : "bottom") );

        $textRotation = 0;
        if ( isset( $alignmentXml->alignment['textRotation'] ) && (int) $alignmentXml->alignment['textRotation'] <= 90 ) {
            $textRotation = (int) $alignmentXml->alignment['textRotation'];
        } elseif ( isset( $alignmentXml->alignment['textRotation'] ) && (int) $alignmentXml->alignment['textRotation'] > 90) {
            $textRotation = 90 - (int) $alignmentXml->alignment['textRotation'];
        }

        $alignment->setTextRotation((int) $textRotation);
        $alignment->setWrapText(self::boolean( isset( $alignmentXml->alignment['wrapText'] ) ? (string) $alignmentXml->alignment['wrapText'] : false ) );
        $alignment->setShrinkToFit(self::boolean( isset( $alignmentXml->alignment['shrinkToFit'] ) ? (string) $alignmentXml->alignment['shrinkToFit'] : false ));
        $alignment->setIndent((int) ( isset( $alignmentXml->alignment['indent'] ) && (string) $alignmentXml->alignment['indent']) > 0 ? (int) ((string) $alignmentXml->alignment['indent']) : 0);
        $alignment->setReadOrder((int) (  isset( $alignmentXml->alignment['readingOrder'] ) &&  (string) $alignmentXml->alignment['readingOrder']) > 0 ? (int) ((string) $alignmentXml->alignment['readingOrder']) : 0);
    }
